import { z } from "zod";
import { InteractionActionEnums } from "./constants/database";
...
export const GetUserAnswersParamsSchema = PaginatedSearchParamsSchema.extend({
  userId: z.string().min(1, { message: "User ID is required." }),
});

export const GetUserTagsSchema = PaginatedSearchParamsSchema.
extend({
  userId: z.string().min(1, { message: "User ID is required." }),
});
